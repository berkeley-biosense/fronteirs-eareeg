...
earbud, we set out to combine all three factors of authentication into a single
step.
...
mastoid (behind the left ear). We chose the AFz ground location to minimize the
chances that our measruement setup caused differences between readings from the
left and right electrodes, , though future systems using one ear only should
test relocating the ground to a site on one ear (e.g., the earlobe). Six
